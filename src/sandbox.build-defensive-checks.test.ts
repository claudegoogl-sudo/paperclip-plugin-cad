 ...
 *
 * 216 closes two "fail loudly when the filter isn't what you think it
...
describe.skipIf(!HAS_BUILD_DEPS)(
  "TEST-216: seccomp build/load chain silent-failure hardening",
  () => {
    // -----------------------------------------------------------------------
    ...
    it("Obs 2 — unresolvable syscall name in kill_syscall aborts the build with errno context", () => {
      ...
      const inject = `    kill_syscall(ctx, "${FAKE}"); /* 216 test injection */\n`;
