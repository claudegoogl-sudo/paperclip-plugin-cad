/**
 * 1089 Ask 2 — mesh-boolean tooling under the REAL bwrap+seccomp sandbox.
 *
 ...
 * FAILURE — the gate cannot be satisfied by a silent skip (1089 SE-3).
 */
...
// 1089 SE-3: a BLOCKING "native code is sandbox-safe / no SIGSYS" sign-off is
...
beforeAll(async () => {
  ...
  // 1089 SE-3: in CI, a bwrap-capable runner that cannot prove the mesh
  // stack is sandbox-importable must FAIL the gate, not skip past it.
  if (IS_CI && HAS_BWRAP && !MESH_OK) {
    throw new Error(
      "TEST-1089 SE-3: mesh-boolean sandbox proof did not execute in CI. " +
...
describe.skipIf(!HAS_BWRAP)("TEST-1089 Ask 2 — mesh-boolean under bwrap+seccomp", () => {
