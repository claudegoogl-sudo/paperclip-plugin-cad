/**
 * Bubblewrap + seccomp performance gates — 114 / 106 §6.2, §6.3.
 *
 * Spec verbatim (§6.2):
 *
...
/**
 * Vitest per-test ceiling. The interleaved gate (1097) runs BOTH the
 ...
 *
 * 1109: the p95 adder = bwrap.p95 − dev_direct.p95, both co-measured
 * interleaved (1097) through the same invokeWorker path, so it is a
 ...
 * decision formula, 1109): ~29 % headroom over the worst observed for
 ...
 * 1109: §6.3 originally gated the median as a ±5 % ratio band, but the
 ...
 *
 * 1109: the original §6.3 grace check gated `bwrap.max − dev_direct.p50`
 ...
 * median-vs-noise contamination 1097/e763fed removed from the p50 path.
 *
 * We instead gate the *interleaved per-iteration* worst adder
 * `max(bwrap_i − dev_i)` over the 1097 paired samples: each bwrap sample
 ...
 * rounded up to the nearest 25, sanity-clamped ≤ 400 (CTO decision, 1109).
 *
...
/**
 * 1097: when set, the bwrap gate captures the dev_direct baseline in the
...
describe.skipIf(!CAPTURE_BASELINE)("TEST-114 §6.2 — dev_direct baseline capture", () => {
...
describe.skipIf(!HAS_BWRAP || CAPTURE_BASELINE)(
  "TEST-114 §6.2/§6.3 — bwrap+seccomp performance gates",
  () => {
    ...
    it(
      ...
      async () => {
        // 1097: build a dev_direct decision in THIS process so the
